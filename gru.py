...
class GRU(torch.nn.Module):
    """
    Recurrent Neural Network (GRU) for Time Series Analysis.
    """
...
def train(X_train, y_train, n_epochs=1000, lr=0.001, save_path=None):
    ...
    # --- Build the model ---
    model = GRU(input_size=1, hidden_size=64, output_size=1)
    summary(model)
    ...
    if (save_path is not None):
        torch.save(model.state_dict(), Path(save_path, 'gru.pth'))
