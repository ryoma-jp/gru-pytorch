...
from gru import train as gru_train
from gru import predict as gru_predict
from gru import evaluate as gru_evaluate
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Impremenation of GRU for Time Series Analysis")
...
def main():
    ...
    # --- Training GRU ---
    model = gru_train(X_train, y_train, save_path=args.output_dir)
    train_pred = gru_predict(model, X_train)
    train_mse = gru_evaluate(y_train, train_pred)
    print(f'Train MSE: {train_mse}')

    # --- Test GRU ---
    test_pred = gru_predict(model, X_test)
    test_mse = gru_evaluate(y_test, test_pred)
